...
describe('PurchaseManager 클래스 - processPurchase', () => {
  ...
  beforeEach(() => {
    ...
    purchaseManager.inventory = new Inventory();
  ...
  afterEach(() => {
    jest.clearAllMocks(); // 모든 모킹된 콘솔 상태 초기화
  });

  test('isPromotionApplied가 true, 증정품을 더 받지 않는 경우, 프로모션 재고 감소', async () => {
    const product = purchaseManager.inventory.getProduct('콜라');
    expect(product.isPromotionApplied).toBe(true);

    Console.readLineAsync.mockImplementationOnce(() => Promise.resolve('Y'));

    await purchaseManager.processPurchase('콜라', 1);
    const updatedProduct = purchaseManager.inventory.getProduct('콜라');
    expect(updatedProduct.promotionQuantity).toBe(9);
    expect(updatedProduct.normalQuantity).toBe(5);
  });
